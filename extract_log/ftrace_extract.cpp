...
const string POSSIBLE_FUNCTIONS_CHARACTERS =
 ...
 public:
  ...
  FtraceLogRow(const FtraceLogRow &row)
      : task(row.task),
        pid(row.pid),
        cpu(row.cpu),
        irqs_off(row.irqs_off),
        need_resched(row.need_resched),
        hardirq_softirq(row.hardirq_softirq),
        preempt_depth(row.preempt_depth),
        migrate_disable(row.migrate_disable),
  ...
  FtraceLogRow(string task, int pid, int cpu, char irqs_off, char need_resched,
               char hardirq_softirq, char preempt_depth, char migrate_disable,
               string timestamp, string duration, string func, string args = "")
      : task(task),
        pid(pid),
        cpu(cpu),
        irqs_off(irqs_off),
        need_resched(need_resched),
        hardirq_softirq(hardirq_softirq),
  ...
  FtraceLogRow(tuple<string, int, int, char, char, char, char, char, string,
                     string, string, string>
                   row) {
    FtraceLogRow(get<0>(row), get<1>(row), get<2>(row), get<3>(row),
                 get<4>(row), get<5>(row), get<6>(row), get<7>(row),
                 get<8>(row), get<9>(row), get<10>(row), get<11>(row));
  }
  FtraceLogRow(tuple<string, int, int, char, char, char, char, char, string,
                     string, string>
                   row) {
    FtraceLogRow(get<0>(row), get<1>(row), get<2>(row), get<3>(row),
                 get<4>(row), get<5>(row), get<6>(row), get<7>(row),
                 get<8>(row), get<9>(row), get<10>(row));
  }
  FtraceLogRow() {}

  // Getters
  string get_task() { return task; }
  int get_pid() { return pid; }
  int get_cpu() { return cpu; }
  char get_irqs_off() { return irqs_off; }
  char get_need_resched() { return need_resched; }
  char get_hardirq_softirq() { return hardirq_softirq; }
  char get_preempt_depth() { return preempt_depth; }
  char get_migrate_disable() { return migrate_disable; }
  string get_timestamp() { return timestamp; }
  string get_duration() { return duration; }
  string get_func() { return func; }
  string get_args() { return args; }
  tuple<string, int, int, char, char, char, char, char, string, string, string,
        string>
  get_tuple() {
    return {task,          pid,
            cpu,           irqs_off,
  ...
  void set_cpu(int cpu) { this->cpu = cpu; }
  void set_irqs_off(char irqs_off) { this->irqs_off = irqs_off; }
  ...
  void set_duration(string duration) { this->duration = duration; }
  void set_func(string func) { this->func = func; }
  void set_args(string args) { this->args = args; }
  void set_tuple(tuple<string, int, int, char, char, char, char, char, string,
                       string, string, string>
                     t) {
  ...
  ostream &print_row(ostream &os = cout, string delim = " | ") const {
    os << task << delim << pid << delim << cpu << delim << irqs_off << delim
       << need_resched << delim << hardirq_softirq << delim << preempt_depth
       << delim << migrate_disable << delim << timestamp << delim << duration
       << delim << func << delim << args << '\n';
    return os;
  }
  ...
  string operator()() const {
    stringstream ss;
    ss << task << " | " << pid << " | " << cpu << " | " << irqs_off << " | "
       << need_resched << " | " << hardirq_softirq << " | " << preempt_depth
       << " | " << migrate_disable << " | " << timestamp << " | " << duration
       << " | " << func << " | " << args << '\n';
    return ss.str();
  }
};
...
class FtraceLog {
 ...
 public:
  // Constructors
  FtraceLog() {}
  FtraceLog(vector<FtraceLogRow> &rows) { this->rows = rows; }
